#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PD3_GameInstance

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "Starbreeze_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass PD3_GameInstance.PD3_GameInstance_C
// 0x0008 (0x05B0 - 0x05A8)
class UPD3_GameInstance_C final : public USBZGameInstance
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x05A8(0x0008)(ZeroConstructor, Transient, DuplicateTransient)

public:
	void ExecuteUbergraph_PD3_GameInstance(int32 EntryPoint);
	void OnFindReplaysComplete(const TArray<struct FSBZReplayInfo>& Replays);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"PD3_GameInstance_C">();
	}
	static class UPD3_GameInstance_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPD3_GameInstance_C>();
	}
};
static_assert(alignof(UPD3_GameInstance_C) == 0x000008, "Wrong alignment on UPD3_GameInstance_C");
static_assert(sizeof(UPD3_GameInstance_C) == 0x0005B0, "Wrong size on UPD3_GameInstance_C");
static_assert(offsetof(UPD3_GameInstance_C, UberGraphFrame) == 0x0005A8, "Member 'UPD3_GameInstance_C::UberGraphFrame' has a wrong offset!");

}

